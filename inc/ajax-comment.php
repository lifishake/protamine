 ...
 * 来源: 破袜子根据大发的代码修改
 */
function protamine_newcomment( $is_grasp ) {
	global $wpdb;
 ...
 * 来源: 破袜子原创
 */
function protamine_additional_comment_show( $email ) {
	?>
...
function ajax_comment_callback(){
    ...
    $dict = $is_grasp ? 'xaja-comment-psarg':'xaja-comment-timbus';
    if ( ! wp_verify_nonce($_POST['nonce'],$dict) )
        die();
	if ( false==(bool)$is_grasp ) {
		$comment_author_email = protamine_newcomment( false );
		protamine_additional_comment_show($comment_author_email); 
	}
	else {
		$comment_author_email = protamine_newcomment( true );
		protamine_additional_grasp_show($comment_author_email); 
	}
    die();
}
