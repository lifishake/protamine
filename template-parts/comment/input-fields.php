...
<?php else : ?>
    <?php
        ...
        if ( $cookie ) {
            $btn = is_page()?'':'<input id="grasp" class="submit" type="button" value="圈阅" name="grasp" wpnonce="'.wp_create_nonce('xaja-comment-psarg').'"/>';
...
<input name="submit" type="<?php echo apply_filters('apip_submit_status', 'submit'); ?>" id="submit" class="submit" value="提交留言" wpnonce="<?php echo wp_create_nonce('xaja-comment-timbus'); ?>" />
